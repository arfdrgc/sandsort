...
sampler2D _ReplaceMap;
sampler2D _ReplaceNoise;
float _ReplaceLerp;
float _ReplaceSize;
...
struct Varyings
{
	...
	float3 vertexColor	   : COLOR; // object-space position
	...
	) : SV_Target
{
	...
	// Base
	half4 albedo = tex2D(_BaseMap, mainTexcoord);
	half4 replace = tex2D(_ReplaceMap, mainTexcoord);
	half4 noise = tex2D(_ReplaceNoise, mainTexcoord * _ReplaceNoise_ST.xy + _ReplaceNoise_ST.zw);

	float zRatio = input.vertexColor.r;
	float noiseValue = noise.r * _ReplaceSize; // scale noise effect

	float lerpValue = smoothstep(_ReplaceLerp - noiseValue, _ReplaceLerp + noiseValue, zRatio);

	albedo = lerp(replace, albedo, lerpValue);
