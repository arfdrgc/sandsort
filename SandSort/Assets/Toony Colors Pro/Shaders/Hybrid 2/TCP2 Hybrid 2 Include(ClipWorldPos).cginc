	...
	) : SV_Target
{
	...
	float showAboveZMask = step(_ZThreshold, positionWS.z);
	float showBelowZMask = 1.0 - showAboveZMask;
	float visibleZ = lerp(showBelowZMask, showAboveZMask, saturate(_ShowAbove));

	float showAboveXMask = step(_XThreshold, positionWS.x);
	float showBelowXMask = showAboveXMask;
	float visibleX = lerp(showBelowXMask, showAboveXMask, saturate(_ShowAbove));

	float visible = visibleZ + visibleX;
	clip(visible - 0.5);
