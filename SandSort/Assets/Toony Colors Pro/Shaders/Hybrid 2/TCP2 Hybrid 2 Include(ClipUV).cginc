	...
	) : SV_Target
{
	...
	float showAboveMask = step(_ClipThreshold, mainTexcoord.x); // 1 if UV.x >= threshold
	float showBelowMask = 1.0 - showAboveMask;                  // 1 if UV.x < threshold
	float visible = lerp(showBelowMask, showAboveMask, saturate(_ShowAbove)); // Picks one based on _ShowAbove

	clip(visible - 0.5); // clip if visible < 0.5
