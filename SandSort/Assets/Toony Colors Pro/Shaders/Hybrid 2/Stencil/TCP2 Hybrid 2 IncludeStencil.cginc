	...
	) : SV_Target
{
	...
	float desaturate = (max(color.r, max(color.g, color.b)) + min(color.r, min(color.g, color.b))) / 2.0;
	color = lerp(desaturate, color, _Saturation);
	
	return half4(color * _Darkness, alpha);
}
