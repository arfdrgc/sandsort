...
//	half4 _Emission3Color;
	...
	half4 _StaticSpecularDirection;
...
half CalculateSpecular(half3 lightDir, half3 viewDir, float3 normal, half specularMap)
{
	...
	#if defined(_STATICSPECULAR)
		float3 halfDir = TCP2_SafeNormalize(_StaticSpecularDirection.xyz);
	#else
	...
	return clamp(spec, _SpecularMin, _SpecularMax);
}
...
float3 ApplyRippleDisplacement(
	float3 positionOS,
	float2 rippleCenter,
	float amplitude,
	float frequency,
...
{
	float2 posXZ = positionOS.xz;
	float dist = distance(posXZ, rippleCenter);
	float wave = sin((dist - time * speed) * frequency) * amplitude;
	positionOS.y += wave;
...
VERTEX_OUTPUT Vertex(Attributes input)
{
		...
		output.texcoords.zw = input.texcoord0.xy;
			...
			#if _RIPPLE
				half3 pos = ApplyRippleDisplacement(input.vertex, _RippleCenter.xy, _RippleAmplitude, _RippleFrequency, _RippleSpeed, _Time.x);
	...
	) : SV_Target
{
	...
	// Emission
		...
		#endif
//		emission += _Emission2Color.rgb + _Emission3Color.rgb;
	#endif
...
#if _SHADERMAN
	color = pow(saturate(color * _FinalFactor), _FinalPower);
	float grayscale = dot(color, float3(0.299, 0.587, 0.114));
	color = lerp(color, grayscale.xxx, _FinalGrayscale);
#endif
