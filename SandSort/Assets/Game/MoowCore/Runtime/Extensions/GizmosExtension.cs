...
namespace Moow {
    public static class GizmosExtensions {
        ...
        public static void DrawText(string text, Vector3 pos, Color textColor, Color backgroundColor, int fontSize = 10, int padding = 2) {
#if UNITY_EDITOR
            GUIStyle style = new GUIStyle();
            style.normal.textColor = backgroundColor;
            style.normal.background = getTexture();
        ...
        public static void DrawIcon(Texture2D texture, Vector3 worldPos, Vector2 size, Vector2 pixelOffset) {
#if UNITY_EDITOR
            Vector3 screenPosition = HandleUtility.WorldToGUIPoint(worldPos);
            Rect rect = new Rect(screenPosition.x - size.x / 2 + pixelOffset.x, screenPosition.y - size.y / 2 + pixelOffset.y, size.x, size.y);
            Handles.BeginGUI();
