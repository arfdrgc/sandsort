...
public class SandCylinderTunables : MonoBehaviour {
    ...
    [Tooltip("Adds a 45-degree diagonal wing to each side of the extraction window, on top of (never instead of) the vertical block window above. Starting from the window's bottom corners, every row higher up reaches one more column further out to the left and right, so the full coverage is a trapezoid that is exactly one block wide at the bottom and widest at the extractionRangeY ceiling. The ceiling itself and the grid's left/right edges are never exceeded. Only which cells are reachable changes — rate, flow caps and maxCellsPerColumnPerTick apply exactly as before. Off = the plain vertical block window (original behaviour).")]
    public bool extractionDiagonalSpread = false;
