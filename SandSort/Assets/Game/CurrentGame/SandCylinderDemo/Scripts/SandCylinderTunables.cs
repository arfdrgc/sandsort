...
public class SandCylinderTunables : MonoBehaviour {
    ...
    [Tooltip("Edge collapse: how strongly sand beside open space (a face, a crater wall, the edge of a drained block) shifts toward that space as a hole rises through it. Such a hole has sand above it and a supported grain diagonally above on the sand side only; the chance that grain fills it is raised from flowSpread (0) to certainty (1). The hole then climbs into the sand body instead of straight up the face column, so a steep face slumps from its upper part instead of standing and translating sideways. Holes in the open bulk, the grid's outer walls and the rest shape are unaffected. 0 = off (the baseline policy exactly). Measured (204x204, 27 cells/frame mouth, 2 passes/frame): a vertical 150-row cliff settles in 850 frames instead of 1424, a drain's tail ends ~35% sooner, the narrow ~75° chimney over the mouth becomes a ~45° crater.")]
    [Range(0f, 1f)] public float edgeCollapse = 1f;
