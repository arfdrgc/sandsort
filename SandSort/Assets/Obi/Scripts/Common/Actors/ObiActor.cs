...
namespace Obi
{
    ...
    public abstract class ObiActor : MonoBehaviour, IObiParticleCollection
    {
        ...
        public void SaveStateToBlueprint(ObiActorBlueprint bp)
        {
            ...
            if (ContainsInvalidTRS(l2sTransform))
            {
                Debug.LogWarning("actorLocalToSolverMatrix contains invalid values. Using identity instead.");
                l2sTransform = Matrix4x4.identity;
            }
        ...
        private bool ContainsInvalidTRS(Matrix4x4 m)
        {
            for (int i = 0; i < 16; i++)
            {
