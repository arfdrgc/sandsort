...
namespace Obi
{
    ...
    {
        // Constraint comparer
        public struct ConstraintComparer<K> : IComparer<K> where K : unmanaged, IConstraint
        {
            public int Compare(K x, K y)
            {
                return x.GetParticle(1).CompareTo(y.GetParticle(1));
        ...
        [BurstCompile]
        public struct CountSortPerFirstParticleJob<K> : IJob where K : unmanaged, IConstraint
        {
            [ReadOnly] [NativeDisableContainerSafetyRestriction] public NativeArray<K> input;
            public NativeArray<K> output;
            [NativeDisableContainerSafetyRestriction] public NativeArray<int> digitCount;
        ...
        public struct SortSubArraysJob<K> : IJobParallelFor where K : unmanaged, IConstraint
        {
            ...
            public void Execute(int workItemIndex)
            {
                int startIndex = workItemIndex > 0 ? NextElementIndex[workItemIndex - 1] : 0;
