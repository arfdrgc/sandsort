...
namespace Obi
{
    public class BurstSolverImpl : ISolverImpl
    {
        ...
        public void SetSimplices(int[] simplices, SimplexCounts counts)
        {
            this.simplices.ResizeUninitialized(simplices.Length);
            for (int i = 0; i < simplices.Length; i++)
                this.simplices[i] = simplices[i];
            ...
            simplexBounds = new Unity.Collections.NativeArray<BurstAabb>(counts.simplexCount, Unity.Collections.Allocator.Persistent);
            cellCoords = abstraction.cellCoords.AsNativeArray<int4>();
        }
