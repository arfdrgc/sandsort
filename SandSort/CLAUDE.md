...
- **Haptics** (`Runtime/Haptic Feedback`, cross-device since 2026-09-25, tested on Xiaomi 15T Pro / HyperOS and Xiaomi T9): always go through `HapticManager` (or CurrentGame's `GameplayHaptics`), never call the native layer directly. Android (`AndroidVibrationEngine`) probes once and uses the best backend the device reports: predefined effects (API 30+, only if reported supported) → amplitude one-shots/waveforms (API 26+) → plain on/off vibrate → `Handheld.Vibrate` (rate-limited, only with no Vibrator). Every vibration is tagged game/media usage (`VibrationAttributes`/`AudioAttributes`). Untagged short clicks count as touch feedback on Android 13+ and HyperOS drops them, so **never remove the tagging**. It never throws: failures step down to the next backend. iOS (`Assets/Plugins/iOS/UnityHaptic.mm`) uses UIFeedbackGenerators and falls back to a rate-limited system vibrate when CoreHaptics reports no haptics support. Details: `TODO.md` → "Cross-device haptics".
